...
-- employees_id 是主键 约束是not null 意思是不为空 且auto_increment 自动增长 
...
-- 因为users表中的年龄不会超出tinyint tinyint不加符号范围在0-255
...
SELECT department_id,first_name FROM employees WHERE NOT(department_id =90);
...
SELECT first_name,salary FROM employees WHERE first_name LIKE '__n%';
...
SELECT first_name,job_id FROM employees WHERE job_id IN('it_prot','av_vp','ad_pres')
...
否则显示D级别
*/
SELECT salary,CASE WHEN salary>20000 THEN 'A'
...
ON l.location_id=d.location_id WHERE city LIKE '_o%';
...
JOIN jobs AS j ON e.department_id=d.department_id
...
WHERE job_id=(SELECT job_id employees WHERE employee_id=141)
